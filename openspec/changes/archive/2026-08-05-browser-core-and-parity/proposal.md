...
The browser build at cropsize.pages.dev is now the product, but it is the weaker half. At proposal time it could not do multi-object mode, silently read only page one of a multi-page PDF, and carried imaging maths hand-ported from the Python original with zero fixture tests. The first apply pass has since added page selection and per-page state, but the multi-object and shared-core gaps remain. The hand port has already produced three real defects: an inverted sign in the deskew projection, a zero-degree bias that reported no tilt on a scan tilted 1.4 degrees, and a mask cleanup that whited out 72.7 percent of a passport's second page.
